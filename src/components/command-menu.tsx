  ...
  return (
    <>
      <p className="fixed bottom-0 left-0 right-0 hidden border-t border-t-muted bg-white p-1 text-center text-sm text-muted-foreground print:hidden xl:block">
        {lang === "pl" ? "Naciśnij" : "Press"}{" "}
        <kbd className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] font-medium text-muted-foreground opacity-100">
          <span className="text-xs">⌘</span>J
        </kbd>{" "}
        {lang === "pl" ? "aby otworzyć menu poleceń" : "to open command menu"}
      </p>
      <Button
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <CommandInput placeholder={lang === "pl" ? "Wyszukaj..." : "Search..."} />
        <CommandList>
          <CommandEmpty>
            {lang === "pl" ? "Nie znaleziono wyników" : "No results found."}
          </CommandEmpty>
          <CommandGroup heading={lang === "pl" ? "Akcje" : "Actions"}>
            <CommandItem
              onSelect={() => {
                setOpen(false);
                window.print();
          ...
          <CommandGroup heading={lang === "pl" ? "Linki" : "Links"}>
            {links.map(({ url, title }) => (
              <CommandItem
                key={url}
                onSelect={() => {
